...
object Flight_Assignment {
  ...
  /**
   * Reads a CSV file into a DataFrame and validate schema.
   */
  def readCSV(filePath: String, schema: StructType)(implicit spark: SparkSession): DataFrame = {
    try {
      ...
      validateSchema(df, schema) // Validate schema for correctness to detect any data issues early
   ...
   *        Summary steps
   *        Step 1: select the required columns( "flightData" and "passengers")
